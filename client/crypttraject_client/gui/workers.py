...
progress + result signals back to the main thread. The pages never call
...
class IngestWorker(_BaseWorker):
    """Parse the data source and compute MinHash signatures locally."""

    done = Signal(dict)   # {record_id: np.ndarray}
    ...
    def run(self):
        try:
            ...
            # Emit one tick immediately so the bar shows movement before the
            # first hash is even computed.
            self.progress.emit(0, total, "Starting...")

            for rid, tokens in adapter.iter_features(extractor):
                sigs[rid] = compute_minhash(tokens, num_perm=self.state.num_perm)
                n = len(sigs)
            ...
            self.log.emit(f"Computed {len(sigs)} MinHash signatures.")
            self.done.emit(sigs)
        except Exception as exc:   # noqa: BLE001
